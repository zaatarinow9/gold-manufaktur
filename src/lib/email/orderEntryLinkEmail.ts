...
import { companyInfo, siteName } from "@/lib/site";

type OrderEntryLinkEmailInput = {
  expiresAt?: string;
  link: string;
};
...
export function buildOrderEntryLinkEmail(input: OrderEntryLinkEmailInput) {
  const expiresAtLabel = formatExpiry(input.expiresAt);
  const subject = "Sicherer Link zur Auftragserfassung | GoldHelwah GmbH";
  const intro =
    "anbei erhalten Sie den sicheren Link zur externen Auftragserfassung fuer Ihren Werkstattauftrag.";
  const expiryLine = expiresAtLabel
    ? `Der Link ist bis ${expiresAtLabel} gueltig.`
    : "Der Link bleibt gueltig, bis er in unserem System deaktiviert oder neu erzeugt wird.";
  const text = [
    "Guten Tag,",
    "",
    `Sie erhalten ${intro}`,
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;border-collapse:separate;border-spacing:0;background:linear-gradient(180deg,#17120d,#090909);border:1px solid rgba(196,154,82,0.24);border-radius:28px;overflow:hidden;">
            <tr>
              <td style="padding:34px 30px 18px 30px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.06);">
                <div style="color:#e8c987;font-size:12px;letter-spacing:0.26em;text-transform:uppercase;margin-bottom:10px;">GoldHelwah GmbH</div>
                <h1 style="margin:0;color:#f7f1e3;font-size:30px;line-height:1.15;">Sicherer Link zur Auftragserfassung</h1>
                <p style="margin:16px auto 0 auto;max-width:520px;color:#c7b99e;font-size:15px;line-height:1.7;">
                  Guten Tag,<br />
                  Sie erhalten ${escapeHtml(intro)}
                </p>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:24px 30px 0 30px;text-align:center;">
                <a href="${escapeHtml(input.link)}" style="display:inline-block;padding:15px 28px;border-radius:999px;background:linear-gradient(135deg,#e8c987,#c49a52);color:#16120d;font-size:15px;font-weight:700;text-decoration:none;">Auftrag erfassen</a>
              </td>
            </tr>
            <tr>
              <td style="padding:24px 30px 0 30px;">
                <div style="border:1px solid rgba(196,154,82,0.2);border-radius:24px;background:rgba(196,154,82,0.08);padding:20px;">
                  <div style="color:#e8c987;font-size:12px;letter-spacing:0.14em;text-transform:uppercase;margin-bottom:10px;">Direkter Link</div>
              ...
              <td style="padding:24px 30px 30px 30px;">
                <div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:18px;">
                  <p style="margin:0;color:#c7b99e;font-size:13px;line-height:1.8;">
                    Bei Rueckfragen erreichen Sie uns unter ${escapeHtml(companyInfo.emailDisplay)} oder ${escapeHtml(companyInfo.phoneDisplay)}.
                  </p>
                </div>
              </td>
            </tr>
